...
def extrapolate_previous_value(line):
    ...
    for index, sequence in enumerate(list(tree[1:])):
        new_value = sequence[0] - tree[index][0]
        extrapolated_values.append(new_value)
        tree[index + 1].insert(0, new_value)
...
if __name__ == "__main__":
    ...
    previous_values = [extrapolate_previous_value(line) for line in get_input('input.txt')]

    print('Answer: ' + str(sum(previous_values)))
