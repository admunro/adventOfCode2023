...
def get_seeds(puzzle_input):
    ...
    pairs = re.findall('\d+ \d+', (' ').join(puzzle_input[0].split(' ')[1:]))
    ...
    for pair in pairs:
        ...
        range_start = int(l[0])
        range_length = int(l[1])

        seeds.append((range_start, range_start + range_length))
...
if __name__ == "__main__":
    ...
    puzzle_input = get_input('testInput.txt')

    seed_ranges = get_seeds(puzzle_input)
    ...
    for seed_range in seed_ranges:
        ...
        soils = [apply_map(seed, seed_to_soil_map) for seed in range(seed_range[0], seed_range[1])]
